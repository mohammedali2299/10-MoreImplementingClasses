...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        h_rect_x1 = intersection_center.x - (width/2)
        h_rect_x2 = intersection_center.x + (width/2)
        h_rect_y1 = intersection_center.y - (letter_thickness/2)
        h_rect_y2 = intersection_center.y + (letter_thickness/2)
        self.h_rect = rg.Rectangle(rg.Point(h_rect_x1, h_rect_y1), rg.Point(h_rect_x2, h_rect_y2))

        v_rect_x1 = intersection_center.x - (letter_thickness/2)
        v_rect_x2 = intersection_center.x + (letter_thickness/2)
        v_rect_y1 = intersection_center.y - (letter_thickness/2)
        v_rect_y2 = intersection_center.y - (letter_thickness/2) + height
        self.v_rect = rg.Rectangle(rg.Point(v_rect_x1, v_rect_y1), rg.Point(v_rect_x2, v_rect_y2))
        #print(self.v_rect)
